...
| `convert_xvc_generator.py` | `ckpt/{xvc_codec,xvc_converter,xvc_prenet}.safetensors` | official `xvc.pt` generator → safetensors loadable by `xvc::{SacCodec, AcousticConverter}` and the `XvcEngine` prenet (official names 1:1, `weight_norm` `g·v/‖v‖` pairs folded into plain conv weights) | ✅ |
